...
def create_jasa_manuscript():
    ...
    All numeric results are read from output/results.json (produced by
    generate_papers.py) so that no result is hard-coded in this generator.
    """
    doc = Document()
    ...
    bic_min_sp, bic_min = R.species_extreme(results, "bicoherence_mean", "min")
    ...
    p_uni = R.fmt_sci_unicode(cdma["p_value"])
    n_rec, n_sp = meta["n_recordings"], meta["n_species"]

    def ici_range(sp):
        d = results["ici"].get(sp)
        return f"{d['median_ms_min']:.1f}\u2013{d['median_ms_max']:.1f}" if d else "n/a"
    ...
    add_jasa_paragraph(doc,
        ...
        f"({n_rec:,} recordings, {n_sp} species). Seven analytical methods\u2014spectrogram analysis, "
        ...
        f"within-species correlation ({cdma['within_mean']:.2f}) than between-species correlation ({cdma['between_mean']:.2f}; "
        f"Mann-Whitney U test, p = {p_uni}), supporting the hypothesis that cetaceans "
        ...
        f"with {R.SPECIES_EN[bic_max_sp].lower()}s exhibiting the highest bicoherence ({R.fmt_bicoh_plain(bic_max)}), "
        "providing evidence for nonlinear frequency interactions relevant to the beat frequency hypothesis. "
        f"Shannon information entropy ranged from {R.fmt_entropy(er['min'])} to {R.fmt_entropy(er['max'])} bits across species, "
    ...
    add_jasa_paragraph(doc,
        ...
        f"repository ({meta['name']}, revision {meta['revision'][:7]}). The database contains {n_rec:,} recordings from "
        f"{n_sp} species. Six target species were selected to represent a range of vocal "
        f"production mechanisms and frequency ranges (TABLE I). Up to {meta['max_samples_per_species']} recordings per "
        f"species were analyzed. The first {meta['analysis_seconds']} s of each recording were resampled to "
        f"{meta['target_sr']//1000} kHz for standardized processing.",
        first_line_indent=Inches(0.5))
    ...
    add_jasa_paragraph(doc,
        ...
        f"range {meta.get('ici_window_ms', [1, 2000])[0]:g}\u2013{meta.get('ici_window_ms', [1, 2000])[1]:,.0f} ms.",
        first_line_indent=Inches(0.5))
    ...
    add_jasa_paragraph(doc,
        ...
        f"FIGS. 1\u20136). {R.SPECIES_EN[cen_min_sp]}s exhibited the lowest spectral centroid ({R.fmt_hz(cen_min)} Hz), "
        "consistent with their specialization in long-distance infrasonic communication "
        f"through the SOFAR channel. {R.SPECIES_EN[cen_max_sp]}s showed the highest centroid "
        f"({R.fmt_hz(cen_max)} Hz), reflecting their broadband click and whistle repertoire. Sperm whales "
        f"({R.fmt_hz(S['Sperm_Whale']['centroid_hz'])} Hz) and killer whales ({R.fmt_hz(S['Killer_Whale']['centroid_hz'])} Hz) occupied intermediate positions.",
        first_line_indent=Inches(0.5))
    ...
    result_data = [
        (
            R.SPECIES_EN[sp],
            R.fmt_hz(S[sp]["centroid_hz"]),
            R.fmt_hz(S[sp]["bandwidth_hz"]),
            R.fmt_entropy(S[sp]["entropy_bits"]),
            R.fmt_sci_unicode(S[sp]["bicoherence_mean"]),
        )
        for sp in order
    ]
    ...
    add_jasa_paragraph(doc,
        ...
        f"Sperm whale recordings showed median ICI values of {ici_range('Sperm_Whale')} ms, "
        "with substantial inter-individual variation consistent with individually distinctive "
        "coda patterns reported by Oliveira et al. (2016). Killer whales showed an "
        f"ICI range of {ici_range('Killer_Whale')} ms, and bottlenose dolphins exhibited "
        f"ICIs of {ici_range('Bottlenose_Dolphin')} ms, reflecting their high-repetition-rate echolocation clicks.",
        first_line_indent=Inches(0.5))
    ...
    _win = meta.get("ici_window_ms", [1, 2000])
    _win_txt = f"{_win[0]:g}\u2013{_win[1]:,.0f}"
    for i, h in enumerate(["Species", "Median ICI (ms)", "Detection window (ms)", "No. clicks"]):
        set_cell_text(table3.rows[0].cells[i], h, bold=True, size=10)

    ici_data = []
    for _sp in ["Sperm_Whale", "Killer_Whale", "Bottlenose_Dolphin"]:
        _di = results["ici"].get(_sp)
        if not _di:
            continue
        ici_data.append((
            R.SPECIES_EN[_sp],
            f"{_di['median_ms_min']:.1f}\u2013{_di['median_ms_max']:.1f}",
            _win_txt,
            f"{_di['n_clicks_min']}\u2013{_di['n_clicks_max']} per recording",
        ))
    ...
    add_jasa_paragraph(doc,
        ...
        f"(FIGS. 10\u201315; TABLE II). {R.SPECIES_EN[bic_max_sp]}s exhibited the highest mean off-diagonal "
        f"bicoherence ({R.fmt_sci_unicode(bic_max)}), greater than {R.SPECIES_EN[bic_min_sp].lower()}s "
        f"({R.fmt_sci_unicode(bic_min)}). The presence of quadratic phase coupling indicates that "
    ...
    add_jasa_paragraph(doc,
        ...
        f"TABLE II). {R.SPECIES_EN[ent_max_sp]}s exhibited the highest entropy ({R.fmt_entropy(ent_max)} bits), reflecting their "
        f"broadband, information-rich click trains. {R.SPECIES_EN[ent_min_sp]}s showed the lowest entropy "
        f"({R.fmt_entropy(ent_min)} bits), consistent with their narrow-band, highly repetitive infrasonic pulses. "
        f"Humpback whales ({R.fmt_entropy(S['Humpback_Whale']['entropy_bits'])} bits) occupied an intermediate position, with their complex "
    ...
    add_jasa_paragraph(doc,
        "Spectral flatness analysis (FIGS. 22\u201327) revealed that "
        f"{R.SPECIES_EN[flat_min_sp].lower()}s ({R.fmt_flatness(flat_min)}) produce the most tonal (structured) signals, "
        f"while {R.SPECIES_EN[flat_max_sp].lower()}s ({R.fmt_flatness(flat_max)}) exhibit the most noise-like characteristics. This "
    ...
    add_jasa_paragraph(doc,
        ...
        f"spectral correlation (mean = {cdma['within_mean']:.2f}, SD = {cdma['within_std']:.2f}) was significantly higher than "
        f"between-species correlation (mean = {cdma['between_mean']:.2f}, SD = {cdma['between_std']:.2f}; Mann-Whitney U test, "
        f"p = {p_uni}). This demonstrates that conspecific recordings share "
    ...
    add_jasa_paragraph(doc,
        ...
        f"are not perfectly orthogonal. The moderate effect size (difference \u2248 {cdma['within_mean'] - cdma['between_mean']:.2f}) "
    ...
    add_jasa_paragraph(doc,
        f"The wide range of Shannon entropy values ({R.fmt_entropy(er['min'])}\u2013{R.fmt_entropy(er['max'])} bits) across species "
    ...
    add_jasa_paragraph(doc,
        ...
        f"synchronized multi-point recordings. Third, the sample size (up to {meta['max_samples_per_species']} recordings "
        f"per species) limits the statistical power for individual-level analyses. Fourth, "
        f"the {meta['target_sr']//1000} kHz resampling rate limits analysis to frequencies below "
        f"{meta['target_sr']//2000} kHz, excluding some high-frequency components of dolphin whistles "
        f"and echolocation clicks. Fifth, to bound memory use and ensure deterministic processing, "
        f"only the first {meta['analysis_seconds']} s of each recording were analyzed, which may "
        f"under-sample temporally sparse vocalizations.",
        first_line_indent=Inches(0.5))
    ...
    conclusions = [
        ...
        f"than between-species correlation (p = {p_uni}), satisfying a "
        ...
        f"(4) Shannon entropy ranges from {R.fmt_entropy(er['min'])} to {R.fmt_entropy(er['max'])} bits across species, reflecting "
    ...
    add_jasa_paragraph(doc,
        ...
        f"(https://huggingface.co/datasets/{meta['name']}). The database contains "
        f"{n_rec:,} recordings from {n_sp} marine mammal species and is freely available for "
...
def create_jasa_cover_letter():
    ...
    cdma = results["cdma"]
    p_uni = R.fmt_sci_unicode(cdma["p_value"])
    bic_sp, bic_v = R.species_extreme(results, "bicoherence_mean", "max")
    ...
    body_paragraphs = [
        ...
        f"as spectral codes, we demonstrate that within-species correlation ({cdma['within_mean']:.2f}) is "
        f"significantly higher than between-species correlation ({cdma['between_mean']:.2f}; Mann-Whitney U test, "
        f"p = {p_uni}). Second, we investigate nonlinear frequency coupling "
        "relevant to the beat frequency hypothesis using bispectral analysis, detecting "
        f"quadratic phase coupling in all six species studied, with {R.SPECIES_EN[bic_sp].lower()}s exhibiting "
        f"the strongest bicoherence ({R.fmt_sci_unicode(bic_v)}).",
