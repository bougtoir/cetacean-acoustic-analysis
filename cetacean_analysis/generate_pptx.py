...
def get_results():
    """Load output/results.json once (single source of truth for all numbers)."""
    global _RESULTS
    if _RESULTS is None:
        _RESULTS = R.load_results()
    return _RESULTS


def _display(species_names, sp):
    """Map a results.json species key (may contain commas) to its display name."""
    return species_names.get(sp) or species_names.get(sp.replace(",", ""), sp.replace(",", "").replace("_", " "))


def _spectral_rows(species_names):
    """Table 1 rows built from results.json (no hard-coded numbers)."""
    res = get_results()
    S = res["species"]
    rows = []
    for sp in res["species_order"]:
        d = S[sp]
        rows.append([
            _display(species_names, sp),
            R.fmt_hz(d["centroid_hz"]),
            R.fmt_hz(d["bandwidth_hz"]),
            R.fmt_hz(d["dominant_hz"]),
            R.fmt_entropy(d["entropy_bits"]),
            R.fmt_bicoh_plain(d["bicoherence_mean"]),
        ])
    return rows


def _ici_rows(species_names):
    """Table 3 rows (ICI summary) built from results.json."""
    ici = get_results()["ici"]
    rows = []
    for sp, d in ici.items():
        rows.append([
            _display(species_names, sp),
            f"{d['median_ms_min']:.1f}\u2013{d['median_ms_max']:.1f}",
            f"{d['n_clicks_min']}\u2013{d['n_clicks_max']}",
        ])
    return rows
...
def make_analysis_pipeline_flow(prs, lang="ja"):
    ...
    _meta = get_results()["dataset"]
    _nrec, _nsp, _khz = _meta["n_recordings"], _meta["n_species"], _meta["target_sr"] // 1000
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    add_shape(slide, Cm(0), Cm(0), SLIDE_WIDTH, Cm(0.3), fill_color=COLOR_TITLE_BG)
    ...
    if lang == "ja":
        steps = [
            ("データ取得", f"Watkins WMMS\n{_nrec:,}録音, {_nsp}種", COLOR_ACCENT1),
            ("前処理", f"音声デコード\nモノラル変換\nリサンプリング ({_khz}kHz)", COLOR_BLUE),
            ("特徴抽出", "スペクトログラム\nICI検出\nバイスペクトル\nエントロピー\n時間構造", COLOR_GREEN),
        ...
        steps = [
            ("Data Acquisition", f"Watkins WMMS\n{_nrec:,} recordings, {_nsp} species", COLOR_ACCENT1),
            ("Preprocessing", f"Audio decode\nStereo→Mono\nResample ({_khz}kHz)", COLOR_BLUE),
...
def make_hypothesis_diagram(prs, lang="ja"):
    ...
    _p = R.fmt_sci_unicode(get_results()["cdma"]["p_value"])
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    add_shape(slide, Cm(0), Cm(0), SLIDE_WIDTH, Cm(0.3), fill_color=COLOR_TITLE_BG)
    ...
    if lang == "ja":
        ...
        cdma_result = f"Result: Within > Between correlation\n(p = {_p}) → Supported"
...
def make_species_overview_diagram(prs, lang="ja"):
    ...
    _S = get_results()["species"]
    if lang == "ja":
        _voc = {
            "Sperm_Whale": ("クリック列 (コーダ)", COLOR_BLUE),
            "Humpback_Whale": ("歌 (Zipf分布)", COLOR_GREEN),
            "Killer_Whale": ("コール・ホイッスル", COLOR_ORANGE),
            "Fin,_Finback_Whale": ("超低周波パルス", COLOR_TEAL),
            "Bottlenose_Dolphin": ("ホイッスル・クリック", COLOR_PURPLE),
            "Beluga,_White_Whale": ("多様な発声", COLOR_RED),
        }
        _names = R.SPECIES_JA
        headers_text = ["種名", "発声タイプ", "重心周波数", "エントロピー"]
    else:
        _voc = {
            "Sperm_Whale": ("Click trains (codas)", COLOR_BLUE),
            "Humpback_Whale": ("Songs (Zipf distribution)", COLOR_GREEN),
            "Killer_Whale": ("Calls & whistles", COLOR_ORANGE),
            "Fin,_Finback_Whale": ("Infrasonic pulses", COLOR_TEAL),
            "Bottlenose_Dolphin": ("Whistles & clicks", COLOR_PURPLE),
            "Beluga,_White_Whale": ("Diverse vocalizations", COLOR_RED),
        }
        _names = R.SPECIES_EN
    ...
    species_info = [
        (
            _names.get(sp, sp),
            _voc[sp][0],
            f"{R.fmt_hz(_S[sp]['centroid_hz'])} Hz",
            f"{R.fmt_entropy(_S[sp]['entropy_bits'])} bits",
            _voc[sp][1],
        )
        for sp in get_results()["species_order"]
    ]
...
def make_conclusion_diagram(prs, lang="ja"):
    ...
    _res = get_results()
    _p = R.fmt_sci_unicode(_res["cdma"]["p_value"])
    _er = _res["entropy_range"]
    _emin, _emax = R.fmt_entropy(_er["min"]), R.fmt_entropy(_er["max"])
    slide = prs.slides.add_slide(prs.slide_layouts[6])
    add_shape(slide, Cm(0), Cm(0), SLIDE_WIDTH, Cm(0.3), fill_color=COLOR_TITLE_BG)
...
def build_presentation(lang="ja"):
    ...
    if lang == "ja":
        ...
        rows = _spectral_rows(species_names)
    ...
    if lang == "ja":
        headers2 = ["指標", "値"]
        _cd = get_results()["cdma"]
        rows2 = [
            ["種内スペクトル相関 (平均)", R.fmt_mean_sd(_cd["within_mean"], _cd["within_std"], 4)],
            ["種間スペクトル相関 (平均)", R.fmt_mean_sd(_cd["between_mean"], _cd["between_std"], 4)],
            ["Mann-Whitney U 統計量", f"{_cd['u_stat']:,.1f}"],
            ["p値", R.fmt_sci_unicode(_cd["p_value"])],
            ["帰無仮説", "棄却 (α = 0.05)"],
            ["結論", "種固有スペクトル符号が存在"],
        ]
        ...
        _cd = get_results()["cdma"]
        rows2 = [
            ["Within-species correlation (mean)", R.fmt_mean_sd(_cd["within_mean"], _cd["within_std"], 4)],
            ["Between-species correlation (mean)", R.fmt_mean_sd(_cd["between_mean"], _cd["between_std"], 4)],
            ["Mann-Whitney U statistic", f"{_cd['u_stat']:,.1f}"],
            ["p-value", R.fmt_sci_unicode(_cd["p_value"])],
    ...
    # ---- Table: ICI summary ----
    if lang == "ja":
        headers3 = ["種", "中央ICI範囲 (ms)", "クリック数範囲/録音"]
        rows3 = _ici_rows(species_names)
        caption3 = "表3. クリック間隔 (ICI) 解析の要約"
        table_title3 = "クリック間隔 (ICI) 解析結果"
    else:
        headers3 = ["Species", "Median ICI range (ms)", "No. clicks/recording"]
        rows3 = _ici_rows(species_names)
    ...
    if os.path.exists(img_path):
        if lang == "ja":
            make_image_slide(prs, "CDMA的直交性: スペクトル相関行列", img_path,
                             f"図. 種内vs種間スペクトル相関の比較（Mann-Whitney U検定, p={_p}）")
        else:
            make_image_slide(prs, "CDMA Orthogonality: Spectral Correlation Matrix", img_path,
                             f"Figure. Within vs between species spectral correlation (Mann-Whitney U, p={_p})")
